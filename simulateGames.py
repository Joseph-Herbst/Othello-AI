        ...
        def update(self):
                ...
                if not self.won:
                        ...
                        if self.player==0:
                                ...
                                for x in range(8):
                                        for y in range(8):
                                                if valid(self.array,0,x,y):
                                                        choices.append([x,y])
                                if choices != []:
                                        event = choice(choices)

                                        if valid(board.array,board.player,event[0],event[1]):
                                                board.boardMove(event[0],event[1])
                        else:
                                ##get action back
                                self.oldarray = self.array
                                mctsearch = mcts(timeLimit=100)########################################
                                try:
                                        if not self.passTest():
                                                valid_move = False
                                                while not valid_move:
                                                        action = mctsearch.search(initialState=OthelloState(self))
                                                        valid_move = valid(self.array, 1, action.x, action.y)
                                                self.array = move(self.array, action.x, action.y, self.player)                           
                                                self.oldarray[action.x][action.y]="b"
                                except:
                                        self.won=True
                        ...
                        if not self.reported:
                                gameOver(computer_score,player_score)
                                self.reported = True
                                printResults()
                                playGame()
##                        print(computer_score>=player_score)
        ...
        def boardMove(self,x,y):
                ...
                self.oldarray = self.array
                self.oldarray[x][y]="w"
                self.array = move(self.array,x,y,0)
        ...
        def dumbMove(self):
                ...
                return Action(dumbChoice[0],dumbChoice[1])
        ...
        def slightlyLessDumbMove(self):
                ...
                for x in range(8):
                        for y in range(8):
                                if valid(self.array,self.player,x,y):
                                        test = move(self.array,x,y,self.player)
                                        boards.append(test)
                                        choices.append([x,y])
                ...
                return Action(choices[bestIndex][0],choices[bestIndex][1])
        ...
        def minimax(self, node, depth, maximizing):
                ...
                for x in range(8):
                        for y in range(8):
                                if valid(self.array,self.player,x,y):
                                        test = move(node,x,y,self.player)
                                        boards.append(test)
                                        choices.append([x,y])
...
##                self.board.array = deepcopy(board.array)
        def getPossibleActions(self):
                choices = []
                for x in range(8):
                        for y in range(8):
                                if valid(self.board.array,self.board.player,x,y):
                                        choices.append(Action(x,y))
                return choices
        def takeAction(self,action=None):
                if action == None:
                        self.board.player = 1-self.board.player
                        return OthelloState(self.board)
                self.board.array = move(self.board.array, action.x, action.y,self.board.player)
                self.board.player = 1-self.board.player
                self.passtest()
                return OthelloState(self.board)
        def isTerminal(self):
                choices1 = self.getPossibleActions()
                self.board.player = 1-self.board.player
                choices2 = self.getPossibleActions()
                self.board.player = 1-self.board.player
        ...
        def passtest(self):
                ...
                if mustPass:
                        self.board.player = 1-self.board.player
                        if self.board.passed==True:
                                self.board.won = True
                        else:
                                self.board.passed = True
                else:
                        self.board.passed = False
                        
        def lessDumbMove(self):
                #Generates all possible choices and boards corresponding to those
                boards = []
                choices = []
                for x in range(8):
                        for y in range(8):
                                if valid(self.board.array,1,x,y):
                                        test = move(self.board.array,x,y,1)
                                        boards.append(test)
                                        choices.append(Action(x,y))
                ...
                bestIndex = 0
                for i in range(len(boards)):
                        score= dumbScore(boards[i],1)
                        if score>bestScore:
                                bestIndex=i
                                bestScore = score
                #Move to the best location based on dumbScore()
##                print(choices[bestIndex])
